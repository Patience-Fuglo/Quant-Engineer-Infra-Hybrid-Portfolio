...
def test_main_call_option(monkeypatch, capsys):
    # Simulate command line: python main.py call --spot 100 --strike 100 --rate 0.01 --vol 0.2 --expiry 1
    args = ["main.py", "call", "--spot", "100", "--strike", "100", "--rate", "0.01", "--vol", "0.2", "--expiry", "1"]
...
def test_main_put_option(monkeypatch, capsys):
    args = ["main.py", "put", "--spot", "100", "--strike", "100", "--rate", "0.01", "--vol", "0.2", "--expiry", "1"]
    monkeypatch.setattr(sys, "argv", args)
    main.main()
    captured = capsys.readouterr()
    assert "Delta" in captured.out or "delta" in captured.out
...
def test_main_invalid_option_type(monkeypatch, capsys):
    args = ["main.py", "invalid", "--spot", "100", "--strike", "100", "--rate", "0.01", "--vol", "0.2", "--expiry", "1"]
    monkeypatch.setattr(sys, "argv", args)
